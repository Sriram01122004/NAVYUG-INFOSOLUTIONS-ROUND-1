...
def decode(message, prefix="", results=None, memo=None):
    if results is None:
        results = []
    if memo is None:
        memo = {}

    if (message, prefix) in memo:
        results.extend(memo[(message, prefix)])
        return results

    if not message:
        results.append(prefix)
        memo[(message, prefix)] = [prefix]
        return results

    local_results = []
    for letter, code in codes.items():
        if message.startswith(code):
            decode(message[len(code):], prefix + letter, results, memo)
            local_results.append(prefix + letter)

    memo[(message, prefix)] = local_results
    return results

if __name__ == "__main__":
    message = input("ENTER THE CODE:").strip()
    results = decode(message)
    for word in sorted(set(results)):
        print(word)
